...
class DashboardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (var project in projects) {
      for (var job in project.jobs) {
        ...
        if (workTypeId.startsWith('og_')) {
          costsByCategory['Obra Gruesa'] =
              (costsByCategory['Obra Gruesa'] ?? 0.0) + job.totalCost;
        } else if (workTypeId.startsWith('of_')) {
          costsByCategory['Obra Fina'] =
              (costsByCategory['Obra Fina'] ?? 0.0) + job.totalCost;
        } else if (workTypeId.startsWith('in_')) {
          costsByCategory['Instalaciones'] =
              (costsByCategory['Instalaciones'] ?? 0.0) + job.totalCost;
        }
      }
    }
    ...
    for (var project in projects) {
      ...
      final projectCost =
          project.jobs.fold<double>(0.0, (sum, job) => sum + job.totalCost);
    ...
    final sortedProjects = List<Project>.from(projects)
      ..sort((a, b) {
        final costA =
            a.jobs.fold<double>(0.0, (sum, job) => sum + job.totalCost);
        final costB =
            b.jobs.fold<double>(0.0, (sum, job) => sum + job.totalCost);
  ...
  Widget _buildSummaryCards(
      BuildContext context, int totalProjects, double totalCost) {
    return Row(
      children: [
        Expanded(
  ...
  Widget _buildCategoryChart(BuildContext context,
      Map<String, double> costsByCategory, double totalCost) {
  ...
  Widget _buildCategoryBar(BuildContext context, String category, double cost,
  ...
  Widget _buildRegionStats(BuildContext context,
      Map<String, int> projectsByRegion, Map<String, double> costsByRegion) {
    return Card(
      elevation: 2,
      child: Padding(
        padding: const EdgeInsets.all(16.0),
